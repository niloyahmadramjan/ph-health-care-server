...
const updateSchedule = async (
	...
	if (
		...
		throw new AppError(
			httpStatus.CONFLICT,
			"Schedule one published and appointment booked cannot be updated",
		);
	}
	...
	if (isAfter(payload.startDateTime, payload.endDateTime)) {
		...
		throw new AppError(
			httpStatus.CONFLICT,
			"Start Date time cannot be after end data time",
		);
	}
	...
	const totalSlots = Math.floor(durationMinute / MINUTES_ALLOCATED_PER_SLOT);
	if (totalSlots < 1) {
		throw new AppError(
			httpStatus.CONFLICT,
...
const deleteSchedule = async (scheduleId: string, user: RequestUser) => {
	...
	if (
		...
		throw new AppError(
			httpStatus.CONFLICT,
			"Schedule onece published and appointment booked cannot be deleted",
		);
	}
